...
- **Publication cadence (`GENBOX_GENERATION_INTERVAL_DAYS`, default 1 = daily):** `GenBox/schedule.py` puts publication slots on a fixed grid anchored at `GENBOX_SCHEDULE_ANCHOR_DATE` (default 1970-01-01), so every worker agrees on the grid with no shared state. The **live channel** is `current_slot_date()` — today on a publication day, otherwise the most recent slot, whose bulletin stays on air — and it is the **only** date allowed to generate; rows (and the decision/video/audio locks) are keyed by the slot, not by the day the generation happens to run. The interval is applied **forward only**: which past channels exist is a fact of storage, never recomputed from the current interval, so changing the interval re-times future publications and leaves the archive untouched.
- **Channel navigation is storage-driven:** `/genbox-channels` returns every date that actually has a decision row (`list_decision_dates`) plus the live slot; the TV's back/next knobs step over **that list** rather than ±1 day, which is what keeps the dial correct across an interval change (the archive keeps whatever cadence it was published on). The knobs clamp at both ends and re-fetch the list at the newest end, so a page left open across a publication picks up the new day. If the endpoint is unreachable the frontend falls back to ±1-day stepping and asks for the live channel without naming a date.
