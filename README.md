...
- **Non-Blocking Generation**: Both the decision text (topic + web research + write) and the video/narration run in background threads guarded by single-flight table locks, so the page never hangs. While today's bulletin is still being prepared the TV shows **no-signal static** ("Tuning in…") and re-polls `/get-string` until the text is ready; it then switches to video automatically once the segment is ready. Status is polled via `/genbox-video-status` and cached per day.
