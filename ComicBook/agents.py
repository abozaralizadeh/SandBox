...
def _anchor_to_style(anchor: str, stack: int = 0) -> str:
    """Translate a 9-zone anchor (+ stack index) into an inline absolute-position style.

    Boxes that share an anchor are nudged apart by `stack` so they don't overlap. Inline styles
    are deliberate: they outrank the viewer template's higher-specificity rules, so the
    agent-chosen placement survives in the served page.
    """
    if anchor not in _ANCHOR_ZONES:
        anchor = "bottom-center"
    v, h = ("middle", "center") if anchor == "center" else anchor.split("-")
    offset = stack * 16  # px between stacked boxes in the same zone
    parts = ["position:absolute", "max-width:62%", "pointer-events:auto", "z-index:3"]
    transforms = []
    if v == "top":
        parts.append(f"top:{6 + offset}px")
    elif v == "bottom":
        parts.append(f"bottom:{6 + offset}px")
    else:  # middle
        parts.append(f"top:calc(50% + {offset}px)")
        transforms.append("translateY(-50%)")
    if h == "left":
        parts.append("left:6px")
    elif h == "right":
        parts.append("right:6px")
    else:  # center
        parts.append("left:50%")
        transforms.append("translateX(-50%)")
    if transforms:
        parts.append("transform:" + " ".join(transforms))
    return ";".join(parts) + ";"


def _render_overlays(overlays: list) -> str:
    """Render Reteller-placed boxes as absolutely-positioned overlays on a panel.

    Appearance reuses the .speech-bubble/.caption-box/.sfx classes (themed in the main
    style block); only placement is per-box, via the inline style from _anchor_to_style.
    """
    ...
    for ov in overlays:
        ...
        if not text:
            continue
        typ = ov.get("type", "speech")
        anchor = ov.get("anchor", "bottom-center")
        stack = anchor_counts.get(anchor, 0)
        anchor_counts[anchor] = stack + 1
        style = _anchor_to_style(anchor, stack)
        if typ == "caption":
            inner = f'<div class="caption-box">{_escape_html(text)}</div>'
        elif typ == "sfx":
            inner = f'<div class="sfx">{_escape_html(text)}</div>'
        else:  # speech / thought
            speaker = (ov.get("speaker") or "").strip()
            label = f"{speaker}: " if speaker else ""
            tail = ov.get("tail", "")
            tail_cls = f" tail-{tail}" if tail in ("up", "down", "left", "right") else ""
            inner = f'<div class="speech-bubble{tail_cls}">{_escape_html(label + text)}</div>'
        boxes.append(f'<div class="overlay-box" style="{style}">{inner}</div>')
    return "".join(boxes)
...
def _assemble_html(
    ...
    has_overlays = any(isinstance(p.get("overlays"), list) for p in panels)
    ...
    # Overlay-mode CSS — only emitted for native retellings (panels carrying "overlays"),
    # so the English page's <style> stays exactly as before.
    overlay_css = ""
    if has_overlays:
        sb = t["speech_bg"]
        overlay_css = (
            "\n.overlay-box { position: absolute; box-sizing: border-box; }"
            "\n.overlay-box .speech-bubble, .overlay-box .caption-box { max-width: 100%; }"
            "\n.overlay-box .caption-box { font-size: 0.62em; }"
            "\n.overlay-box .speech-bubble { position: relative; }"
            f"\n.overlay-box .speech-bubble.tail-down::after {{ content:''; position:absolute; top:100%; left:16px; border:6px solid transparent; border-top-color:{sb}; }}"
            f"\n.overlay-box .speech-bubble.tail-up::after {{ content:''; position:absolute; bottom:100%; left:16px; border:6px solid transparent; border-bottom-color:{sb}; }}"
            f"\n.overlay-box .speech-bubble.tail-left::after {{ content:''; position:absolute; right:100%; top:50%; transform:translateY(-50%); border:6px solid transparent; border-right-color:{sb}; }}"
            f"\n.overlay-box .speech-bubble.tail-right::after {{ content:''; position:absolute; left:100%; top:50%; transform:translateY(-50%); border:6px solid transparent; border-left-color:{sb}; }}"
            "\n@media (max-width: 600px) {"
            " .overlay-box { position: static !important; max-width: 100% !important; transform: none !important; margin: 3px 0; }"
            " .panel:has(.overlay-box) { display: flex; flex-direction: column; }"
            " .panel:has(.overlay-box) img { height: auto; }"
            " }"
        )
    ...
    for idx, p in enumerate(panels):
        ...
        if overlays is not None:
            # Overlay mode (native retellings): the Reteller places each box itself.
            text_section = _render_overlays(overlays)
        else:
            # Legacy mode (English master): caption at top, dialogue/sfx stacked at the bottom.
            caption_section = ""
            if caption:
            ...
            text_section = caption_section + overlay_section
...
.comic-branding {{ color: #888; font-size: 0.75em; margin-top: 8px; font-family: sans-serif; }}{rtl_css}{overlay_css}
@media (max-width: 600px) {{
...
def _apply_reteller_output(panels: list, recap: str, teaser: str, native: dict, title: str = "") -> tuple:
    """Overlay natively-written text boxes onto the shared panels, matched by panel number.

    The English panels own the fixed image_url/size; only the text overlays change per language.
    English text must NEVER leak onto a target-language page: a panel the Reteller didn't cover
    gets empty overlays (art only), not the English skeleton.
    """
    ...
    for i, p in enumerate(panels):
        ...
        overlays = match.get("overlays", []) if isinstance(match, dict) else []
        if not isinstance(overlays, list):
            overlays = []
        if match is None:
...
YOU ARE ALSO THE LETTERER — PLACE EVERY BOX:
For each panel, decide how many text boxes it needs and WHERE each one sits, so the lettering fits the art and reads in the natural order for your language. Give every box an "anchor" chosen from this 9-zone grid:
  top-left      top-center      top-right
  middle-left   center          middle-right
  bottom-left   bottom-center   bottom-right
Placement rules:
- Prefer the top and bottom thirds. Keep faces and the central subject clear — avoid "center" except for a big sound effect on an otherwise empty middle.
- Put a character's speech bubble on the side where that character stands (use the positions described in story_context).
- READING ORDER MATTERS. The reader must hit the boxes in the right sequence. Italian reads LEFT to RIGHT, top to bottom — first speaker on the left. Persian/Farsi reads RIGHT to LEFT, top to bottom — so the FIRST thing said goes on the RIGHT, the reply on the LEFT. Order your boxes accordingly.
- Captions (narration) usually sit along the top; sound effects go where the impact is.
- Optionally add a "tail" of "up", "down", "left", or "right" to a speech box to point toward its speaker.
Several boxes may share an anchor (they will stack neatly), but spread them across the panel when you can.
...
- Each speech box names its speaker in the "speaker" field, in the target script (e.g., "JUNIPER" → "جونیپر"). Translate descriptive labels too: "MERCHANT 1" → "بازرگان ۱" / "MERCANTE 1", "CROWD VOICES" → "صداهای جمعیت" / "VOCI DALLA FOLLA". Do NOT repeat the speaker name inside "text" — the letterer adds it.
  ...
  "panels": [
    {
      "number": 1,
      "overlays": [
        {"type": "caption", "text": "...", "anchor": "top-center"},
        {"type": "speech", "speaker": "جونیپر", "text": "...", "anchor": "bottom-right", "tail": "down"},
        {"type": "speech", "speaker": "رِن", "text": "...", "anchor": "middle-left", "tail": "left"},
        {"type": "sfx", "text": "بوم!", "anchor": "center"}
      ]
    }
  ],
...
- "type" is one of "caption", "speech", or "sfx". A "speech" box MUST include "speaker". "anchor" is one of the nine zones above. "tail" is optional.
- A panel may have an empty "overlays" list (a silent, art-only panel) or several boxes. Bubble COUNT is yours to choose — do not feel bound to the number of English lines.
- "text" holds only the native line (no speaker prefix). All text, speaker labels, and glossary values must be in the target script.
