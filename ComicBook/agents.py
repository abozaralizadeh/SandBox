...
def run_comic_pipeline(target_date: datetime) -> Dict[str, Any]:
    ...
    async def _audit_arc_style(_run) -> None:
        """Check whether the arc's reference sheet actually renders as its declared style, and
        on failure STRENGTHEN THE STYLE SPEC — never swap the sheet.

        THE ANCHOR IS IMMUTABLE ONCE PANELS EXIST. This runs after the chain, so the episode's
        panels have already been drawn from `character_sheet_url`. Replacing it here (which is
        what this used to do) breaks two things at once: the page then shows a reference image
        the art was never drawn from, and the NEXT episode is drawn from a different anchor, so
        the cast visibly changes between episodes — precisely the drift the sheet exists to
        prevent.

        Nothing is lost by leaving it alone. Style no longer travels through the sheet:
        `compose_image_prompt` injects the full `render_directive` into EVERY panel prompt in
        code, so the useful product of a failed audit is a stronger directive, which every
        remaining panel of the arc then renders under. To change the look of a running arc,
        set COMICBOOK_RESTYLE_ARC=true — that re-commissions the style and drops the sheet
        BEFORE the chain runs, so the new anchor and the panels are made together.
        ...
        # ONE pass. No regeneration: see the docstring — the anchor the panels were drawn
        # from is immutable.
        try:
            ...
            logger.warning("STYLE AUDIT unavailable (%s) — leaving the arc untouched.",
                           str(exc)[:200])
            return

        logger.info("STYLE AUDIT: verdict=%s score=%.2f worst=%s tells=%s",
                     audit.verdict, audit.overall_score, audit.worst_axis,
        ...
        if audit.verdict == "fail":
            ...
            # directive. It applies to every panel from here on, and to any sheet a later
            # restyle builds — but NOT to the anchor this episode was drawn from.
            ...
            if audit.conceit_advice.strip():
                ...
                dirty = True
            logger.info(
                "STYLE AUDIT failed (worst axis: %s) — strengthened the style spec for the rest "
                "of the arc. The character anchor is deliberately left as-is: the published "
                "panels were drawn from it.", audit.worst_axis,
            )
        else:
            logger.info("STYLE AUDIT passed — '%s' renders as declared.", card.style_name)

        # Mark the arc audited on EVERY path — pass or fail — so this runs once per arc rather
        ...
        try:
            ...
            update_arc_metadata(arc["RowKey"], style_audit_version=STYLE_CARD_VERSION)
            arc.update(style_audit_version=STYLE_CARD_VERSION)
        except Exception as exc:
