...
# Image generation must FAIL FAST: a healthy gpt-image call returns in ~70-90s, so a tight
# timeout with NO client-level retries keeps a degraded/over-capacity gateway from hanging the
# whole comic run for many minutes per panel. App-level retries are deliberately minimal; the
# caller (generate_panel_image) handles persistent failure with a placeholder + circuit breaker.
_IMAGE_TIMEOUT_SECONDS = float(os.environ.get("COMICBOOK_IMAGE_TIMEOUT", "120"))
